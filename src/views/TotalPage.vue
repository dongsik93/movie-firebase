<template>
  <v-container>
    ...
    <v-layout row wrap mw-700 style="font-family: 'Poor Story', cursive; ">
        <v-flex v-for="movie,i in calData" style="margin-bottom:350px; margin-right:150px">
            <div class="panel" >
...
export default {
  ...
  computed: {
    ...
    numOfPages() {
      return Math.ceil(this.length / this.dataPerPage)
    },
    calData() {
      return this.totalMovies.slice(this.startOffset, this.endOffset)
    },
  },
  watch:{
    '$route' : 'getToken',
  ...
  methods : {
    getToken(){
      if(store.state.accessToken){
        ...
        this.totalMovies = store.state.totalMovies
        this.length = this.totalMovies.length
      }
    },
  },
}
